...
| light | rgba rgb | * | / | Defines Thing emitting light.<br />The parameter is RGBA followed by RGB color vector. The RGB overrides the color components in first RGBA parameter. The 4th component in RGBA (i.e. alpha) represents alpha and min & max light radius.<br/>There can be max 64 lights and lightintensity defined in the level.<br />i.e. 64 Things can have either light or lightintensity parameter set.|
| lightintensity | rgb | Actor, Player | / | The actor/player's extra light (spot light).<br/>The limitation for this parm is the same as param *light*.<br/>*Note: the engine also sets thing flag 0x1 - `EmitsLight`.*|
